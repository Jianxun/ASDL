...
def test_convert_document_inline_pin_bindings_merge() -> None:
    doc = AsdlDocument(
        modules={
            "top": ModuleDecl(
                instances={
                    "M1": "nfet (D:VDD G:VIN S:VSS)",
                    "M3": "nfet",
                },
                nets={
                    "$VDD": ["M3.D"],
    ...
    assert set(nets) == {"VDD", "VIN", "VSS"}
    vdd_endpoints = [(ep.inst.data, ep.pin.data) for ep in nets["VDD"].endpoints.data]
    vss_endpoints = [(ep.inst.data, ep.pin.data) for ep in nets["VSS"].endpoints.data]
    vin_endpoints = [(ep.inst.data, ep.pin.data) for ep in nets["VIN"].endpoints.data]

    assert vdd_endpoints == [("M3", "D"), ("M1", "D")]
    assert vss_endpoints == [("M3", "S"), ("M1", "S")]
    assert vin_endpoints == [("M1", "G")]


def test_convert_document_inline_pin_binding_overlap_emits_error() -> None:
    yaml_content = "\n".join(
        [
            "modules:",
            "  top:",
...
def test_convert_document_inline_pin_bindings_create_ports() -> None:
    doc = AsdlDocument(
        modules={
            "top": ModuleDecl(
                instances={"M1": "nfet (D:$VDD G:$VIN S:$VSS)"},
                nets={"$VDD": []},
            )
        },
        devices={
    ...
    assert {net.name_attr.data for net in nets} == {"VDD", "VIN", "VSS"}
