...
def test_parse_string_rejects_import_only() -> None:
    yaml_content = "\n".join(
        [
            "imports:",
            "  gf: gf180/primitives.asdl",
...
def test_parse_string_rejects_bad_import_namespace() -> None:
    yaml_content = "\n".join(
        [
            "imports:",
            "  bad-key: gf180/primitives.asdl",
            "modules:",
            "  top:",
            "    nets:",
            "      $OUT:",
    ...
    diag = diagnostics[0]
    assert diag.code == "PARSE-003"
    assert "imports" in diag.message
...
def test_parse_string_rejects_non_string_import_path() -> None:
    ...
    diag = diagnostics[0]
    assert diag.code == "PARSE-003"
    assert "imports.gf" in diag.message


def test_parse_string_rejects_duplicate_import_namespace() -> None:
    yaml_content = "\n".join(
        [
            "imports:",
            "  gf: gf180/primitives.asdl",
    ...
    assert "duplicate key" in diag.message.lower()
