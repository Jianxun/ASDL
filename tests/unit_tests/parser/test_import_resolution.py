...
def test_resolve_env_expansion_missing_var_emits_error(
...
def test_resolve_env_expansion_empty_value_emits_error(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    monkeypatch.delenv("ASDL_LIB_PATH", raising=False)
    entry_file = tmp_path / "entry.asdl"
    _write_stub(entry_file)
    monkeypatch.setenv("EMPTY_IMPORT_ROOT", "")
...
def test_resolve_env_lib_path_expansion(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    entry_file = tmp_path / "entry.asdl"
    _write_stub(entry_file)
    home_root = tmp_path / "homelib"
    home_root.mkdir()
    env_root = tmp_path / "envlib"
    env_root.mkdir()
    target = env_root / "dep.asdl"
    _write_stub(target)
    ...
    monkeypatch.setenv(
        "ASDL_LIB_PATH",
        f"~/{home_root.name}{os.pathsep}$ASDL_IMPORT_ROOT",
    )
