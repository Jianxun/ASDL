...
- [X] **Task: Stabilize test suite after `Locatable` refactoring** ✅
  - [X] Fixed critical `NameError` and location-tracking bugs in `ASDLParser`. ✅
  - [X] Updated the entire parser test suite to align with improved validation logic. ✅
  - [X] Corrected outdated assertions and invalid test data that were causing failures. ✅
  - [X] Successfully isolated all remaining test failures to the `Elaborator` component. ✅
  - [X] Committed all fixes to the repository, creating a stable foundation for elaborator development. ✅
