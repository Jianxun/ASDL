    ...
    def verify_(self) -> None:
        ...
        for op in self.body.block.ops:
            ...
            if isinstance(op, ModuleOp):
                continue
            raise VerifyException(
                f"unexpected op '{op.name}' in asdl_cir.design; expected import or module"
            )


@irdl_op_definition
class ImportOp(IRDLOperation):
    name = "asdl_cir.import"
    ...
    def verify_(self) -> None:
        ...
        for op in self.body.block.ops:
            ...
            if isinstance(op, ViewOp):
                continue
            raise VerifyException(
                f"unexpected op '{op.name}' in asdl_cir.module; expected port or view"
            )
    ...
    def verify_(self) -> None:
        ...
        for op in self.body.block.ops:
            if isinstance(op, VarOp):
                ...
                raise VerifyException(
                    f"unexpected op '{op.name}' in asdl_cir.view body"
                )
...
class SubcktRefOp(IRDLOperation):
    ...
    def verify_(self) -> None:
        ...
        if module is None:
            raise VerifyException("subckt_ref must be nested within an asdl_cir.module")
